...
export function HeroVisual() {
  return (
    <div className="relative w-full max-w-2xl mx-auto">
      ...
      <div className="hidden md:block absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-primary/20 rounded-full blur-[100px] mix-blend-screen" />

      {/* Main Dashboard Mockup — always visible */}
      <motion.div
        animate={{ y: [-10, 10, -10] }}
      ...
      >
        ...
        <div className="p-4 sm:p-5 md:p-6 flex flex-col gap-4">
          <div className="flex justify-between items-start mb-2">
            <div>
              <h3 className="text-base sm:text-lg font-bold text-foreground">Advanced Contract Drafting</h3>
              <p className="text-xs text-muted-foreground">Module 4: Intellectual Property</p>
            </div>
            <div className="px-2.5 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-bold tracking-wider uppercase border border-primary/20 shrink-0">
              65% Completed
            </div>
          </div>
            ...
            {[
              ...
              <div key={i} className={cn("p-3 rounded-xl border flex items-center gap-3", lesson.active ? "bg-primary/5 border-primary/30" : "bg-card border-border/40")}>
                ...
                <div className="min-w-0">
                  <p className={cn("text-xs font-semibold truncate", lesson.active ? "text-foreground" : "text-muted-foreground")}>{lesson.title}</p>
      ...
      {/* Floating Certificate — hidden on mobile to prevent overlap */}
      <motion.div
        animate={{ 
          y: [-15, 15, -15],
          rotate: [10, 15, 10]
        ...
        className={cn(
          "hidden md:block absolute -bottom-[5%] right-0 w-[50%] p-1 rounded-xl",
      ...
      {/* Floating Review/Social Proof — hidden on mobile to prevent overlap */}
      <motion.div
        animate={{ 
          y: [15, -15, 15],
          rotate: [-5, -10, -5]
        ...
        className={cn(
          "hidden md:block absolute bottom-[15%] left-0 w-[55%] p-4 rounded-2xl",
